...
const Trading = () => {
  ...
  const SortDropdown = ({ value, onChange }: { value: SortMode; onChange: (v: SortMode) => void }) => (
    <Select value={value} onValueChange={(v) => onChange(v as SortMode)}>
      <SelectTrigger className="w-auto h-7 text-xs px-2.5 gap-1 border-none" style={{ background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.7)' }}>
        <SelectValue />
      </SelectTrigger>
      <SelectContent>
  ...
  return (
    <div className="relative px-4 pt-14 pb-28 space-y-6 max-w-md mx-auto">
      ...
      <div className="relative z-20 text-center mb-2">
        <h1 className="text-[22px] font-bold tracking-wide" style={{ color: '#FFFFFF' }}>{t('nav.trading')}</h1>
        <p className="text-[13px] mt-1" style={{ color: 'rgba(255,255,255,0.55)' }}>Discover collectors and trade stickers</p>
      </div>
      ...
      <div className="relative z-20 flex items-center justify-center gap-4">
        <Sheet open={friendsSheetOpen} onOpenChange={setFriendsSheetOpen}>
          <SheetTrigger asChild>
            <DiscoveryButton badge={friendsBadgeCount}>
              <Users className="h-6 w-6 stroke-[2.2]" style={{ color: 'rgba(207,227,255,0.7)' }} />
            </DiscoveryButton>
          </SheetTrigger>
          <SheetContent side="right" className="w-full sm:max-w-md page-bg border-l" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
            <SheetHeader><SheetTitle style={{ color: '#FFFFFF' }}>{t('home.friendsWhoCanHelp')}</SheetTitle></SheetHeader>
            <div className="mt-4 space-y-3 overflow-y-auto max-h-[calc(100vh-120px)]">
              {friendsMatchesLoading ? (
                ...
                <div className="flex flex-col items-center py-8 text-center">
                  <Users className="h-12 w-12 mb-3" style={{ color: 'rgba(255,255,255,0.2)' }} />
                  <p style={{ color: 'rgba(255,255,255,0.5)' }}>{t('home.noFriendDuplicates')}</p>
                </div>
              ) : (
                <>
        ...
        <Sheet open={citySheetOpen} onOpenChange={setCitySheetOpen}>
          <SheetTrigger asChild>
            <DiscoveryButton badge={cityBadgeCount}>
              <MapPin className="h-6 w-6 stroke-[2.2]" style={{ color: 'rgba(207,227,255,0.7)' }} />
            </DiscoveryButton>
          </SheetTrigger>
          <SheetContent side="right" className="w-full sm:max-w-md page-bg border-l" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
            <SheetHeader><SheetTitle style={{ color: '#FFFFFF' }}>{city ? t('home.peopleInCityWhoCanHelp', { city }) : t('home.localMatches')}</SheetTitle></SheetHeader>
            <div className="mt-4 space-y-3 overflow-y-auto max-h-[calc(100vh-120px)]">
              {matchesLoading ? (
                <div className="space-y-3">{[1, 2, 3].map((i) => <Skeleton key={i} className="h-20 w-full rounded-xl" style={{ background: 'rgba(255,255,255,0.06)' }} />)}</div>
              ) : !city ? (
                ...
                <div className="flex flex-col items-center py-8 text-center">
                  <MapPin className="h-12 w-12 mb-3" style={{ color: 'rgba(255,255,255,0.2)' }} />
                  <p style={{ color: 'rgba(255,255,255,0.5)' }}>{t('home.noCollectorsInCity', { city })}</p>
                </div>
              ) : usersWithMatches.length === 0 ? (
                <div className="flex flex-col items-center py-8 text-center">
                  <Users className="h-12 w-12 mb-3" style={{ color: 'rgba(255,255,255,0.2)' }} />
                  <p className="mb-2" style={{ color: 'rgba(255,255,255,0.5)' }}>{t('home.noMatchesInCity', { city })}</p>
                  <p className="text-sm" style={{ color: 'rgba(255,255,255,0.35)' }}>{t('home.noMatchesInCityDetail')}</p>
                </div>
              ) : (
                <>
                  <div className="flex items-center gap-1.5 text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>
        ...
        <Sheet open={uniSheetOpen} onOpenChange={setUniSheetOpen}>
          <SheetTrigger asChild>
            {universityId ? (
              <DiscoveryButton badge={uniBadgeCount}>
                <GraduationCap className="h-6 w-6 stroke-[2.2]" style={{ color: 'rgba(207,227,255,0.7)' }} />
              </DiscoveryButton>
            ) : (
              <DiscoveryButton badge={0} style={{ opacity: 0.5 }}>
                <GraduationCap className="h-6 w-6 stroke-[2.2]" style={{ color: 'rgba(207,227,255,0.7)' }} />
              </DiscoveryButton>
            )}
          </SheetTrigger>
          <SheetContent side="right" className="w-full sm:max-w-md page-bg border-l" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
            <SheetHeader><SheetTitle style={{ color: '#FFFFFF' }}>{t('home.uniWhoCanHelp')}</SheetTitle></SheetHeader>
            <div className="mt-4 space-y-3 overflow-y-auto max-h-[calc(100vh-120px)]">
              {!universityId ? (
                <div className="flex flex-col items-center py-8 text-center">
                ...
                <div className="flex flex-col items-center py-8 text-center">
                  <GraduationCap className="h-12 w-12 mb-3" style={{ color: 'rgba(255,255,255,0.2)' }} />
                  <p style={{ color: 'rgba(255,255,255,0.5)' }}>{t('home.noUniMatches')}</p>
                </div>
              ) : (
                <>
                  <div className="flex items-center gap-1.5 text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>
      ...
      <div className="relative z-20 flex items-center justify-center gap-4 -mt-4">
        <span className="w-14 text-center text-[9px] font-bold uppercase tracking-[0.15em]" style={{ color: 'rgba(255,255,255,0.45)' }}>{t('trading.friends')}</span>
      ...
      <section className="relative z-20 space-y-3">
        ...
        {isLoading ? (
          <div className="space-y-2">
            {[1, 2].map((i) => <Skeleton key={i} className="h-16 w-full rounded-xl" style={{ background: 'rgba(255,255,255,0.06)' }} />)}
          </div>
        ) : activeTrades.length === 0 ? (
          <div className="premium-panel p-6 flex flex-col items-center text-center">
            <p className="text-[14px]" style={{ color: 'rgba(255,255,255,0.5)' }}>{t('trading.noActiveTrades')}</p>
            <p className="text-[12px] mt-2" style={{ color: 'rgba(255,255,255,0.35)' }}>{t('trading.findCollectors')}</p>
          </div>
        ) : (
          <div className="space-y-2">
            {activeTrades.map((trade) => (
              <button
                key={trade.id}
                className="w-full flex items-center justify-between p-4 rounded-[16px] text-left transition-all duration-150 active:scale-[0.98]"
                style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}
              ...
              >
                <div>
                  ...
                  <p className="text-[12px] mt-0.5" style={{ color: 'rgba(255,255,255,0.45)' }}>{t('trading.chatAvailable')}</p>
                </div>
                <div className="flex items-center gap-1 text-[12px] font-medium px-3 py-1.5 rounded-xl" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.10)', color: 'rgba(255,255,255,0.85)' }}>
                  {t('trading.open')}
